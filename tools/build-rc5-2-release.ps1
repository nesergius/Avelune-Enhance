...
function Stop-StalePackagedProcesses {
    ...
    try {
        $processes = @(Get-CimInstance Win32_Process -Filter "Name = 'Avelune Enhance.exe'" -ErrorAction SilentlyContinue)
...
try {
    ...
    try {
        ...
        foreach ($spec in $probeSpecs) {
            ...
            if (-not $uiData.metrics.passed) {
                ...
                $uiProbeFailures += "$($spec.Name): $failedNames (exit $uiExitCode)"
            }
            else {
    ...
    $releaseData = [ordered]@{
        ...
        current = [ordered]@{
            ...
            public = $false
            notes = "RC6 adds verified official models, local Auto Profile, two-stage Neural Restore, persistent Smart Queue, compatible metadata/ICC preservation, GPU AutoTune with OOM recovery, and multi-viewport visual QA screenshots. Public download remains disabled until Authenticode signing and final security scanning."
            files = @(
                [ordered]@{
                    type = "setup"
